...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="bg-white dark:bg-gray-800 shadow-sm rounded-xl p-6 border border-gray-200 dark:border-gray-700">
        <div class="flex items-start justify-between">
            <div>
                <h1 class="text-3xl font-bold text-gray-900 dark:text-white mb-2">Register for Event</h1>
    ...
    <!-- Registration Form -->
    <form method="POST" action="{{ route('events.registrations.store', $event->id) }}" enctype="multipart/form-data" class="bg-white dark:bg-gray-800 shadow-sm rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
        @csrf
            ...
            <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" name="coming_from_city" id="coming_from_city" value="{{ old('coming_from_city') }}" placeholder="Enter your city" class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg shadow-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-white">
                    </div>
                    <div>
                        ...
                        <select name="travel_mode" id="travel_mode" class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg shadow-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-white">
                            ...
                            <option value="car" {{ old('travel_mode') == 'car' ? 'selected' : '' }}>Car</option>
                            <option value="train" {{ old('travel_mode') == 'train' ? 'selected' : '' }}>Train</option>
                            <option value="flight" {{ old('travel_mode') == 'flight' ? 'selected' : '' }}>Flight</option>
                            <option value="bus" {{ old('travel_mode') == 'bus' ? 'selected' : '' }}>Bus</option>
                            <option value="other" {{ old('travel_mode') == 'other' ? 'selected' : '' }}>Other</option>
                        </select>
                    </div>
                    <div>
                        <label for="arrival_date" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Arrival Date</label>
                        <input type="date" name="arrival_date" id="arrival_date" value="{{ old('arrival_date') }}" class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg shadow-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-white">
                    </div>
                    <div>
                        <label for="arrival_time" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Arrival Time</label>
                        <input type="time" name="arrival_time" id="arrival_time" value="{{ old('arrival_time') }}" class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg shadow-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-white">
                    </div>
                </div>
            </div>
        ...
        <div class="px-6 py-4 bg-gray-50 dark:bg-gray-700/30 border-t border-gray-200 dark:border-gray-700 flex items-center justify-between">
            ...
            <button type="submit" class="bg-indigo-600 text-white px-6 py-3 rounded-lg hover:bg-indigo-700 dark:bg-indigo-500 dark:hover:bg-indigo-600 font-medium transition-colors shadow-sm hover:shadow">
                Register for Event
            </button>
        </div>
    </form>
