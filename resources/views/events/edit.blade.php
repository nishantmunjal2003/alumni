...
<div class="mb-6">
    ...
    <p class="text-gray-600">Update event details and manage batch invitations</p>
</div>

<form action="{{ route('admin.events.update', $event) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-lg shadow-lg p-6 lg:p-8">
    @csrf
    @method('PUT')
    ...
    <div class="mb-6">
        ...
        <textarea name="description" id="description" rows="5" required
            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition-all duration-300">{{ old('description', $event->description) }}</textarea>
        @error('description')
            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
        @enderror
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        ...
        <div>
            ...
            @if($event->image)
                <div class="mb-2">
                    <img src="{{ asset('storage/' . $event->image) }}" alt="{{ $event->title }}" class="w-full h-32 object-cover rounded-lg">
                </div>
            @endif
        ...
        <div class="lg:col-span-2 space-y-4">
            <div>
                ...
                <input type="datetime-local" name="event_date" id="event_date" value="{{ old('event_date', $event->event_date ? $event->event_date->format('Y-m-d\TH:i') : '') }}" required
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition-all duration-300">
                @error('event_date')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="location" id="location" value="{{ old('location', $event->location) }}" placeholder="Enter location or paste Google Maps link"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition-all duration-300">
                ...
                <div>
                    ...
                    <input type="text" name="venue" id="venue" value="{{ old('venue', $event->venue) }}" placeholder="Enter venue name (e.g., Auditorium Hall)"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition-all duration-300">
            ...
            @forelse($graduationYears as $year)
                <label class="flex items-center space-x-2 cursor-pointer hover:bg-purple-50 p-2 rounded">
                    <input type="checkbox" name="target_graduation_years[]" value="{{ $year }}" 
                        {{ in_array($year, old('target_graduation_years', $event->target_graduation_years ?? [])) ? 'checked' : '' }}
        ...
        @if($event->invites_sent)
            <div class="mt-4 p-3 bg-yellow-50 rounded-lg border border-yellow-200">
                <p class="text-sm text-yellow-800">
                    <strong>⚠️ Note:</strong> Invitations have already been sent for this event. If you add new batches, invitations will be sent to those batches only.
                </p>
            </div>
        @else
            <div class="mt-4 p-3 bg-blue-50 rounded-lg border border-blue-200">
                <p class="text-sm text-blue-800">
                    <strong>📧 Automatic Invitations:</strong> When you update this event with selected batches, email invitations will be automatically sent to all alumni in those batches.
                </p>
            </div>
        @endif
    </div>
    ...
    <div class="flex gap-4 pt-6 border-t border-gray-200">
        ...
        <a href="{{ route('events.show', $event) }}" class="bg-gray-500 text-white px-8 py-3 rounded-lg shadow-md hover:shadow-lg hover:-translate-y-0.5 transition-all duration-300 font-semibold">
            Cancel
        </a>
    </div>
</form>
