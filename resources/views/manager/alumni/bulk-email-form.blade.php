...
<div class="max-w-4xl mx-auto space-y-4 sm:space-y-6 px-4 sm:px-0">
    <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 sm:gap-0">
        ...
        <a href="{{ route('manager.alumni.index', request()->except(['page'])) }}" class="text-sm text-indigo-600 hover:text-indigo-800 inline-flex items-center gap-1">
    ...
    <div class="bg-white shadow rounded-lg p-4 sm:p-6">
        ...
        @if($recipientCount > 0)
            ...
            <form method="POST" action="{{ route('manager.alumni.email.bulk.send') }}" class="space-y-6">
                @csrf
                ...
                <div class="flex flex-col sm:flex-row sm:justify-end gap-3 sm:gap-4 pt-6 border-t">
                    <a href="{{ route('manager.alumni.index', request()->except(['page'])) }}" class="w-full sm:w-auto text-center bg-gray-200 text-gray-700 px-6 py-2.5 rounded-md hover:bg-gray-300 transition-colors touch-manipulation">
                        Cancel
                    </a>
        ...
        @else
            <div class="text-center py-12">
                ...
                <div class="mt-6">
                    <a href="{{ route('manager.alumni.index') }}" class="bg-indigo-600 text-white px-6 py-2 rounded-md hover:bg-indigo-700 transition-colors inline-block">
                        Back to Directory
                    </a>
                </div>
            </div>
