        ...
        @foreach($alumni as $alumnus)
            <div class="bg-gray-50 border border-gray-200 rounded-lg p-4">
                ...
                <div class="flex gap-2">
                    <a href="{{ route('manager.alumni.show', $alumnus->id) }}" class="flex-1 text-center bg-indigo-600 text-white px-3 py-2 rounded-md hover:bg-indigo-700 text-xs font-medium transition-colors touch-manipulation">
                        View
                    </a>
                </div>
            </div>
                ...
                <thead class="bg-gray-50">
                    <tr>
                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-48">
                            ...
                            @endphp
                            <a href="{{ route('manager.alumni.index', array_merge(request()->except(['sort_by', 'sort_order', 'page']), ['sort_by' => 'name', 'sort_order' => $nextOrder])) }}" class="flex items-center gap-1 hover:text-indigo-600 transition-colors group">
                                Name
                                @if($isActive)
                        ...
                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-40">
                            ...
                            @endphp
                            <a href="{{ route('manager.alumni.index', array_merge(request()->except(['sort_by', 'sort_order', 'page']), ['sort_by' => 'course', 'sort_order' => $nextOrder])) }}" class="flex items-center gap-1 hover:text-indigo-600 transition-colors group">
                                Course
                                @if($isActive)
                                    @if($currentSortOrder === 'asc')
                                        <svg class="w-4 h-4 text-indigo-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-20">
                            ...
                            @endphp
                            <a href="{{ route('manager.alumni.index', array_merge(request()->except(['sort_by', 'sort_order', 'page']), ['sort_by' => 'passing_year', 'sort_order' => $nextOrder])) }}" class="flex items-center gap-1 hover:text-indigo-600 transition-colors group">
                                Year
                                @if($isActive)
                                    @if($currentSortOrder === 'asc')
                                        <svg class="w-4 h-4 text-indigo-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-32 max-w-[8rem]">
                            ...
                            @endphp
                            <a href="{{ route('manager.alumni.index', array_merge(request()->except(['sort_by', 'sort_order', 'page']), ['sort_by' => 'company', 'sort_order' => $nextOrder])) }}" class="flex items-center gap-1 hover:text-indigo-600 transition-colors group">
                                Company
                                @if($isActive)
                                    @if($currentSortOrder === 'asc')
                                        <svg class="w-4 h-4 text-indigo-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-4 py-3">
                                <div class="flex gap-1.5">
                                    <a href="{{ route('manager.alumni.show', $alumnus->id) }}" class="inline-flex items-center px-2.5 py-1 text-xs font-medium text-white bg-indigo-600 hover:bg-indigo-700 rounded transition-colors whitespace-nowrap">
                                        View
                                    </a>
                                </div>
                            </td>
