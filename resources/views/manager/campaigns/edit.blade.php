...
@section('title', 'Edit Fundraising Campaign')

@section('content')
<div class="max-w-4xl mx-auto space-y-6">
    <div>
        <h1 class="text-3xl font-bold text-gray-900">Edit Fundraising Campaign</h1>
        <p class="mt-2 text-sm text-gray-600">Update campaign details and donation information.</p>
    </div>

    <form method="POST" action="{{ route('manager.campaigns.update', $campaign->id) }}" enctype="multipart/form-data" class="bg-white shadow rounded-lg p-6 sm:p-8">
        @csrf
        @method('PUT')
        <div class="space-y-6">
            <!-- Campaign Title -->
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="8" required 
                    ...
                    class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('description', $campaign->description) }}</textarea>
            ...
            <div>
                ...
                @if($campaign->image)
                    <div class="mb-3">
                        <img src="{{ asset('storage/' . $campaign->image) }}" alt="Current campaign image" class="w-full max-w-md h-48 object-cover rounded-lg border border-gray-200">
                        <p class="mt-1 text-xs text-gray-500">Current image</p>
                    </div>
                @endif
                <input type="file" name="image" id="image" accept="image/*" 
                    class="mt-1 block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-indigo-50 file:text-indigo-700 hover:file:bg-indigo-100">
                <p class="mt-1 text-xs text-gray-500">Upload a new image to replace the current one (max 2MB). Recommended size: 1200x600px</p>
                @error('image')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="url" name="donation_link" id="donation_link" value="{{ old('donation_link', $campaign->donation_link) }}" 
                    placeholder="https://payment-gateway.com/donate/..."
                    class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                <p class="mt-1 text-xs text-gray-500">Add or update the payment gateway or donation platform link.</p>
                @error('donation_link')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <select name="status" id="status" required 
                    class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                    <option value="draft" {{ old('status', $campaign->status) == 'draft' ? 'selected' : '' }}>Draft (Save for later)</option>
                    <option value="published" {{ old('status', $campaign->status) == 'published' ? 'selected' : '' }}>Published (Make it live)</option>
                </select>
