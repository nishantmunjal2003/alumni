            ...
            <div class="bg-white shadow rounded-lg p-4 space-y-3">
                <div class="flex items-start justify-between">
                    <div class="flex items-start gap-3 flex-1">
                        @php
                            $serialNumber = ($pendingProfiles->currentPage() - 1) * $pendingProfiles->perPage() + $index + 1;
                        @endphp
                        <span class="text-sm font-semibold text-teal-600 mt-1">#{{ $serialNumber }}</span>
                        <div class="flex-1">
                ...
                <div class="flex flex-col gap-2 pt-2 border-t border-gray-200">
                    <a href="{{ route('dataentry.profiles.show', $profile->id) }}" class="w-full inline-flex items-center justify-center px-4 py-2 text-sm font-medium text-white bg-teal-600 hover:bg-teal-700 rounded-md transition-colors touch-manipulation">
                    ...
                    <div class="grid grid-cols-2 gap-2">
                        <form method="POST" action="{{ route('dataentry.profiles.approve', $profile->id) }}" class="inline">
                            @csrf
                        ...
                        <form method="POST" action="{{ route('dataentry.profiles.block', $profile->id) }}" class="inline">
                            @csrf
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-3 py-2">
                                <div class="flex gap-1 flex-nowrap">
                                    <a href="{{ route('dataentry.profiles.show', $profile->id) }}" class="inline-flex items-center px-2 py-1 text-xs font-medium text-white bg-teal-600 hover:bg-teal-700 rounded transition-colors whitespace-nowrap flex-shrink-0">
                                    ...
                                    <form method="POST" action="{{ route('dataentry.profiles.approve', $profile->id) }}" class="inline flex-shrink-0">
                                        @csrf
                                    ...
                                    <form method="POST" action="{{ route('dataentry.profiles.block', $profile->id) }}" class="inline flex-shrink-0">
                                        @csrf
