...
<div class="max-w-4xl mx-auto">
    <div class="bg-white rounded-lg shadow-lg p-8">
        <h1 class="text-3xl font-bold mb-2 text-gray-900">Register for Event</h1>
        <p class="text-gray-600 mb-6">{{ $event->title }}</p>

        <form action="{{ route('events.register.store', $event) }}" method="POST" enctype="multipart/form-data">
            @csrf
            ...
            <div class="mb-8 p-6 bg-purple-50 rounded-lg border border-purple-200">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div>
                        ...
                        <input type="date" name="arrival_date" id="arrival_date" value="{{ old('arrival_date') }}" min="{{ $event->event_date->format('Y-m-d') }}"
                    ...
                    <div>
                        ...
                        <input type="text" name="coming_from_city" id="coming_from_city" value="{{ old('coming_from_city') }}" placeholder="Enter city name"
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500">
            ...
            <div class="mb-8 p-6 bg-blue-50 rounded-lg border border-blue-200">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div class="flex items-center">
                        <input type="checkbox" name="needs_stay" id="needs_stay" value="1" {{ old('needs_stay') ? 'checked' : '' }}
                    ...
                    <div class="flex items-center">
                        <input type="checkbox" name="coming_with_family" id="coming_with_family" value="1" {{ old('coming_with_family') ? 'checked' : '' }}
                            class="rounded text-purple-600 focus:ring-purple-500 w-5 h-5">
                ...
                <div>
                    ...
                    <textarea name="return_journey_details" id="return_journey_details" rows="3" placeholder="Please share your return journey details..."
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500">{{ old('return_journey_details') }}</textarea>
                        ...
                        @foreach($alumni as $alumnus)
                            <label class="flex items-center space-x-2 cursor-pointer hover:bg-purple-50 p-2 rounded">
                                <input type="checkbox" name="friends[]" value="{{ $alumnus->id }}" 
                                    {{ in_array($alumnus->id, old('friends', [])) ? 'checked' : '' }}
            ...
            <div class="mb-8 p-6 bg-yellow-50 rounded-lg border border-yellow-200">
                ...
                <div class="mb-4">
                    ...
                    <textarea name="memories_description" id="memories_description" rows="4" placeholder="Share your memories with friends..."
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500">{{ old('memories_description') }}</textarea>
                </div>
