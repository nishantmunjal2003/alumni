...
@section('title', 'Send Campaign Announcement')

@section('content')
<div class="max-w-4xl mx-auto space-y-4 sm:space-y-6 px-4 sm:px-0">
    <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 sm:gap-0">
        <div>
            <h1 class="text-2xl sm:text-3xl font-bold">Send Campaign Announcement</h1>
            <p class="mt-1 text-sm sm:text-base text-gray-600">
                <span class="font-semibold text-indigo-600">{{ $recipientCount }}</span> 
                active alumni will receive this campaign announcement
            </p>
        </div>
        <a href="{{ route('admin.campaigns.index') }}" class="text-sm text-indigo-600 hover:text-indigo-800 inline-flex items-center gap-1">
    ...
    <div class="bg-gradient-to-r from-indigo-500 to-purple-600 rounded-lg shadow-lg p-6 text-white">
        ...
        <p class="text-indigo-100 text-sm mb-4">{{ Str::limit($campaign->description, 150) }}</p>
    ...
    <div class="bg-white shadow rounded-lg p-4 sm:p-6">
        ...
        <form method="GET" action="{{ route('admin.campaigns.email', $campaign->id) }}" id="filter-form" class="space-y-4">
            ...
            <div class="flex gap-3">
                ...
                <a href="{{ route('admin.campaigns.email', $campaign->id) }}" 
    ...
    <div class="bg-white shadow rounded-lg p-4 sm:p-6">
        ...
        @if($recipientCount > 0)
            <div class="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                ...
                @else
                    <p class="text-xs text-blue-700 mb-3">
                        All active alumni will receive this campaign announcement email.
                    </p>
                @endif
            ...
            <form method="POST" action="{{ route('admin.campaigns.email.send', $campaign->id) }}" class="space-y-6">
                @csrf
                ...
                <div>
                    ...
                    <input type="text" name="subject" id="subject" 
                        value="{{ old('subject', 'New Fundraising Campaign: ' . $campaign->title) }}" 
                        required 
                        class="w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" 
                ...
                <div>
                    ...
                    <textarea name="message" id="message" rows="12" required 
                        class="w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" 
                        placeholder="Enter your campaign announcement message here...">{{ old('message', "Dear Alumni,\n\nWe are excited to announce a new fundraising campaign:\n\n" . $campaign->title . "\n\n" . Str::limit($campaign->description, 200) . "\n\n" . ($campaign->donation_link ? "You can support this cause by visiting: " . $campaign->donation_link . "\n\n" : "") . "Thank you for your continued support!\n\nBest regards,\nAlumni Portal Team") }}</textarea>
                    <p class="mt-1 text-sm text-gray-500">
                        The message will be personalized with each recipient's name. You can include campaign details, donation link, and any other relevant information.
                    </p>
                </div>
                ...
                <div class="flex flex-col sm:flex-row sm:justify-end gap-3 sm:gap-4 pt-6 border-t">
                    ...
                    <button type="submit" 
                        ...
                        onclick="return confirm('Are you sure you want to send this campaign announcement to {{ $recipientCount }} recipients?')">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        @else
            <div class="text-center py-12">
                ...
                <p class="mt-1 text-sm text-gray-500">There are no active alumni with approved profiles to send the campaign announcement to.</p>
                <div class="mt-6">
                    <a href="{{ route('admin.campaigns.index') }}" 
